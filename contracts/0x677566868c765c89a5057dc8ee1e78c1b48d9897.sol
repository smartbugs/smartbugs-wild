...
contract GeneScienceInterface {
    ...
    /// @dev given genes of LinglongCat 1 & 2, return a genetic combination - may have a random factor
...
/// @title A facet of LinglongCatCore that manages special access privileges.
/// @author CybEye (http://www.cybeye.com/us/index.jsp)
/// @dev See the LinglongCatCore contract documentation to understand how the various contract facets are arranged.
contract LinglongCatACL {
    // This facet controls access control for LinglongCats. There are four roles managed here:
    //
    //     - The CEO: The CEO can reassign other roles and change the addresses of our dependent smart
    //         contracts. It is also the only role that can unpause the smart contract. It is initially
    //         set to the address that created the smart contract in the LinglongCatCore constructor.
    //
    //     - The CFO: The CFO can withdraw funds from LinglongCatCore and its auction contracts.
    //
    //     - The COO: The COO can release gen0 LinglongCats to auction, and mint promo cats.
    //
...
/// @title Base contract for LinglongCat. Holds all common structs, events and base variables.
/// @author Axiom Zen (https://www.axiomzen.co)
/// @dev See the LinglongCatCore contract documentation to understand how the various contract facets are arranged.
contract LinglongCatBase is LinglongCatACL {
	/*** EVENTS ***/

	/// @dev The Birth event is fired whenever a new LinglongCat comes into existence. This obviously
	///  includes any time a LinglongCat is created through the giveBirth method, but it is also called
	///  when a new gen0 LinglongCat is created.
	event Birth(address owner, uint256 LinglongCatId, uint256 matronId, uint256 sireId, uint256 genes, uint256 generation);

	/// @dev Transfer event as defined in current draft of ERC721. Emitted every time a LinglongCat
	///  ownership is assigned, including births.
	event Transfer(address from, address to, uint256 tokenId);

	/*** DATA TYPES ***/

	/// @dev The main LinglongCat struct. Every LinglongCat in LinglongCat is represented by a copy
	///  of this structure, so great care was taken to ensure that it fits neatly into
	///  exactly two 256-bit words. Note that the order of the members in this structure
	///  is important because of the byte-packing rules used by Ethereum.
	///  Ref: http://solidity.readthedocs.io/en/develop/miscellaneous.html
	struct LinglongCat {
		// The LinglongCat's genetic code is packed into these 256-bits, the format is
		// sooper-sekret! A LinglongCat's genes never change.
		uint256 genes;

		// The timestamp from the block when this LinglongCat came into existence.
		uint64 birthTime;

		// The minimum timestamp after which this LinglongCat can engage in breeding
		...
		// Set to the ID of the sire LinglongCat for matrons that are pregnant,
		// zero otherwise. A non-zero value here is how we know a LinglongCat
		// is pregnant. Used to retrieve the genetic material for the new
		// LinglongCat when the birth transpires.
		uint32 siringWithId;

		// Set to the index in the cooldown array (see below) that represents
		// the current cooldown duration for this LinglongCat. This starts at zero
		// for gen0 LinglongCats, and is initialized to floor(generation/2) for others.
	...
	/// @dev An array containing the LinglongCat struct for all LinglongCats in existence. The ID
	///  of each LinglongCat is actually an index into this array. Note that ID 0 is a negaLinglongCat,
	///  the unLinglongCat, the mythical beast that is the parent of all gen0 LinglongCats. A bizarre
	///  creature that is both matron and sire... to itself! Has an invalid genetic code.
	///  In other words, LinglongCat ID 0 is invalid... ;-)
	LinglongCat[] LinglongCats;

	/// @dev A mapping from LinglongCat IDs to the address that owns them. All LinglongCats have
	///  some valid owner address, even gen0 LinglongCats are created with a non-zero owner.
	...
	/// @dev A mapping from LinglongCatIDs to an address that has been approved to call
	///  transferFrom(). Each LinglongCat can only have one approved address for transfer
	///  at any time. A zero value means no approval is outstanding.
	mapping (uint256 => address) public LinglongCatIndexToApproved;

	/// @dev A mapping from LinglongCatIDs to an address that has been approved to use
	///  this LinglongCat for siring via breedWith(). Each LinglongCat can only have one approved
	///  address for siring at any time. A zero value means no approval is outstanding.
	mapping (uint256 => address) public sireAllowedToAddress;

	/// @dev The address of the ClockAuction contract that handles sales of LinglongCats. This
	...
	/// @dev Assigns ownership of a specific LinglongCat to an address.
	function _transfer(address _from, address _to, uint256 _tokenId) internal {
		// Since the number of LinglongCats is capped to 2^32 we can't overflow this
		ownershipTokenCount[_to]++;
		// transfer ownership
		LinglongCatIndexToOwner[_tokenId] = _to;
		// When creating new LinglongCats _from is 0x0, but we can't account that address.
		if (_from != address(0)) {
			ownershipTokenCount[_from]--;
			// once the LinglongCat is transferred also clear sire allowances
		...
		returns (uint)
	{
		...
		// New LinglongCat starts with the same cooldown as parent gen/2
...
/// @title The external contract that is responsible for generating metadata for the LinglongCats,
...
/// @title The facet of the LinglongCats core contract that manages ownership, ERC-721 (draft) compliant.
/// @dev Ref: https://github.com/ethereum/EIPs/issues/721
///  See the LinglongCatCore contract documentation to understand how the various contract facets are arranged.
contract LinglongCatOwnership is LinglongCatBase, ERC721 {
    ...
    /// @dev Checks if a given address is the current owner of a particular LinglongCat.
    /// @param _claimant the address we are validating against.
    /// @param _tokenId LinglongCat id, only valid when > 0
    function _owns(address _claimant, uint256 _tokenId) internal view returns (bool) {
        return LinglongCatIndexToOwner[_tokenId] == _claimant;
    }

    /// @dev Checks if a given address currently has transferApproval for a particular LinglongCat.
    /// @param _claimant the address we are confirming LinglongCat is approved for.
    /// @param _tokenId LinglongCat id, only valid when > 0
    ...
    ///  _approve() and transferFrom() are used together for putting LinglongCats on auction, and
    ///  there is no value in spamming the log with Approval events in that case.
    function _approve(uint256 _tokenId, address _approved) internal {
        LinglongCatIndexToApproved[_tokenId] = _approved;
    }

    /// @notice Returns the number of LinglongCats owned by a specific address.
    ...
    /// @notice Transfers a LinglongCat to another address. If transferring to a smart
    ///  contract be VERY CAREFUL to ensure that it is aware of ERC-721 (or
    ///  LinglongCats specifically) or your LinglongCat may be lost forever. Seriously.
    /// @param _to The address of the recipient, can be a user or contract.
    /// @param _tokenId The ID of the LinglongCat to transfer.
    /// @dev Required for ERC-721 compliance.
    function transfer(
        address _to,
        uint256 _tokenId
        ...
        whenNotPaused
    {
        ...
        // The contract should never own any LinglongCats (except very briefly
        // after a gen0 cat is created and before it goes on auction).
        require(_to != address(this));
        // Disallow transfers to the auction contracts to prevent accidental
        // misuse. Auction contracts should only take ownership of LinglongCats
    ...
    /// @notice Grant another address the right to transfer a specific LinglongCat via
    ///  transferFrom(). This is the preferred flow for transfering NFTs to contracts.
    /// @param _to The address to be granted transfer approval. Pass address(0) to
    ///  clear all approvals.
    /// @param _tokenId The ID of the LinglongCat that can be transferred if this call succeeds.
    /// @dev Required for ERC-721 compliance.
    function approve(
        address _to,
        uint256 _tokenId
    ...
    /// @notice Transfer a LinglongCat owned by another address, for which the calling address
    ///  has previously been granted transfer approval by the owner.
    /// @param _from The address that owns the LinglongCat to be transfered.
    /// @param _to The address that should take ownership of the LinglongCat. Can be any address,
    ///  including the caller.
    /// @param _tokenId The ID of the LinglongCat to be transferred.
    /// @dev Required for ERC-721 compliance.
    ...
    /// @notice Returns the address currently assigned ownership of a given LinglongCat.
    /// @dev Required for ERC-721 compliance.
    ...
    /// @param _tokenId The ID number of the LinglongCat whose metadata should be returned.
...
/// @title A facet of LinglongCatCore that manages LinglongCat siring, gestation, and birth.
/// @author Axiom Zen (https://www.axiomzen.co)
/// @dev See the LinglongCatCore contract documentation to understand how the various contract facets are arranged.
contract LinglongCatBreeding is LinglongCatOwnership {
    ...
    // Keeps track of number of pregnant LinglongCats.
    ...
    /// @dev Set the cooldownEndTime for the given LinglongCat, based on its current cooldownIndex.
    ///  Also increments the cooldownIndex (unless it has hit the cap).
    /// @param _cat A reference to the LinglongCat in storage which needs its timer started.
    ...
    /// @notice Grants approval to another user to sire with one of your LinglongCats.
    /// @param _addr The address that will be able to sire with your LinglongCat. Set to
    ///  address(0) to clear all siring approvals for this LinglongCat.
    /// @param _sireId A LinglongCat that you own that _addr will now be able to sire with.
    ...
    /// @param _matron A reference to the LinglongCat struct of the potential matron.
    /// @param _matronId The matron's ID.
    /// @param _sire A reference to the LinglongCat struct of the potential sire.
    ...
    function _isValidMatingPair(
        ...
        returns(bool)
    {
        // A LinglongCat can't breed with itself!
        if (_matronId == _sireId) {
            return false;
        }

        // LinglongCats can't breed with their parents.
        ...
        // LinglongCats can't breed with full or half siblings.
    ...
    function _breedWith(uint256 _matronId, uint256 _sireId) internal {
        // Grab a reference to the LinglongCats from storage.
        LinglongCat storage sire = LinglongCats[_sireId];
        LinglongCat storage matron = LinglongCats[_matronId];
        ...
        // Every time a LinglongCat gets pregnant, counter is incremented.
    ...
    /// @notice Breed a LinglongCat you own (as matron) with a sire that you own, or for which you
    ///  have previously been given Siring approval. Will either make your cat pregnant, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBirth()
    /// @param _matronId The ID of the LinglongCat acting as matron (will end up pregnant if successful)
    /// @param _sireId The ID of the LinglongCat acting as sire (will begin its siring cooldown if successful)
        ...
        whenNotPaused
    {
        ...
        //   because the owner of a LinglongCat on auction is the auction house, and the
        ...
        // All checks passed, LinglongCat gets pregnant!
...
contract SiringClockAuction is ClockAuction {
    ...
    /// require sender to be LinglongCatCore contract.
    /// @param _tokenId - ID of token to auction, sender must be owner.
    /// @param _startingPrice - Price of item (in wei) at beginning of auction.
    /// @param _endingPrice - Price of item (in wei) at end of auction.
...
/// @title Clock auction modified for sale of LinglongCats
/// @notice We omit a fallback function to prevent accidental sends to this contract.
contract SaleClockAuction is ClockAuction {
    ...
    // Tracks last 5 sale price of gen0 LinglongCat sales
...
/// @title Handles creating auctions for sale and siring of LinglongCats.
///  This wrapper of ReverseAuction exists only so that users can create
///  auctions with only one transaction.
contract LinglongCatAuction is LinglongCatBreeding {

    // @notice The auction contract variables are defined in LinglongCatBase to allow
    //  us to refer to them in LinglongCatOwnership to prevent accidental transfers.
    // `saleAuction` refers to the auction for gen0 and p2p sale of LinglongCats.
    // `siringAuction` refers to the auction for siring rights of LinglongCats.
    ...
    /// @dev Put a LinglongCat up for auction.
        ...
        whenNotPaused
    {
        ...
        // If LinglongCat is already on any auction, this will throw
        // because it will be owned by the auction contract.
        require(_owns(msg.sender, _LinglongCatId));
        // Ensure the LinglongCat is not pregnant to prevent the auction
        // contract accidentally receiving ownership of the child.
        // NOTE: the LinglongCat IS allowed to be in a cooldown.
        require(!isPregnant(_LinglongCatId));
        _approve(_LinglongCatId, saleAuction);
        // Sale auction throws if inputs are invalid and clears
        // transfer and sire approval after escrowing the LinglongCat.
    ...
    /// @dev Put a LinglongCat up for auction to be sire.
    ///  Performs checks to ensure the LinglongCat can be sired, then
    ...
    /// to the LinglongCatCore contract. We use two-step withdrawal to
...
/// @title all functions related to creating LinglongCats
...
/// @title LinglongCats: Collectible, breedable, and oh-so-adorable LinglongCats on the Ethereum blockchain.
/// @dev The main LinglongCats contract, keeps track of cats so they don't wander around and get lost.
contract LinglongCatCore is LinglongCatMinting {
/* contract LinglongCatCore { */
    // This is the main LinglongCats contract. In order to keep our code seperated into logical sections,
    // we've broken it up in two ways. First, we have several seperately-instantiated sibling contracts
    // that handle auctions and our super-top-secret genetic combination algorithm. The auctions are
    // seperate since their logic is somewhat complex and there's always a risk of subtle bugs. By keeping
    // them in their own contracts, we can upgrade them without disrupting the main contract that tracks
    // LinglongCat ownership. The genetic combination algorithm is kept seperate so we can open-source all of
    ...
    //
    //      - LinglongCatBase: This is where we define the most fundamental code shared throughout the core
    //             functionality. This includes our main data storage, constants and data types, plus
    //             internal functions for managing these items.
    //
    //      - LinglongCatAccessControl: This contract manages the various addresses and constraints for operations
    //             that can be executed only by specific roles. Namely CEO, CFO and COO.
    //
    //      - LinglongCatOwnership: This provides the methods required for basic non-fungible token
    //             transactions, following the draft ERC-721 spec (https://github.com/ethereum/EIPs/issues/721).
    //
    //      - LinglongCatBreeding: This file contains the methods necessary to breed LinglongCats together, including
    //             keeping track of siring offers, and relies on an external genetic combination contract.
    //
    //      - LinglongCatAuctions: Here we have the public methods for auctioning or bidding on LinglongCats or siring
    ...
    /// @notice Creates the main LinglongCats smart contract instance.
    function LinglongCatCore() public {
        ...
        // start with the mythical LinglongCat 0 - so we don't have generation-0 parent issues
