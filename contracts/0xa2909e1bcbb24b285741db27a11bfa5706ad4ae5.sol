  ...
  /* Crowdsale start time */
	...
	*/
	function receiveETH(address beneficiary) internal {
		...
		uint manusToSend = bonus(msg.value.mul(MANUS_PER_ETHER).div(1 ether)); // Compute the number of manus to send
	...
	*/
	function finalize() onlyOwner public {
		...
		if (manusSentToEther < MIN_CAP && now < endTime + 15 days) throw; // If MIN_CAP is not reached donors have 15days to get refund before we can finalise

		if (!multisigEther.send(this.balance)) throw; // Move the remaining Ether to the multisig address

		uint remains = manus.balanceOf(this);
		if (remains > 0) { // Burn the rest of manus
	...
	*/
	...
	function getRemainManus() onlyOwner public {
		...
		manus.transfer(owner, remains); // Transfer manus right now
