...
contract PreSquirrelICO is Crowdsale, TimedCrowdsale, WhitelistedCrowdsale, IndividuallyCappedCrowdsale, AllowanceCrowdsale {
  ...
  // maximum (cumulative) purchase == 15 ETH
  ...
  // separately deployed Membership contract since it will be used by multiple (3)
  // ICO contracts of the different ICO stages (preSquirrel, preICO, ICO)
  ...
  // override to check atleast min amount eth is being paid
  ...
  function _processPurchase(address _beneficiary, uint _tokenAmount)
    internal
  {
    // 30% bonus added in here, so it is not accounted for when counting in saved per user contributions
