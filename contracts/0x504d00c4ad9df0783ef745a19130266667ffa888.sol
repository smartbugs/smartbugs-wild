...
//****************
    ...
    mapping (uint256 => LBdatasets.Player) public plyr_;   // (pID => data) player data
    mapping (uint256 => mapping (uint256 => LBdatasets.PlayerRounds)) public plyrRnds_;    // (pID => rID => data) player round data by player id & round id
    mapping (uint256 => mapping (bytes32 => bool)) public plyrNames_; // (pID => name => bool) list of names a player owns.  (used so you can change your display name amongst any name you own)
//****************
// ROUND DATA 
//****************
    mapping (uint256 => LBdatasets.Round) public round_;   // (rID => data) round data
    mapping (uint256 => mapping(uint256 => uint256)) public rndTmEth_;      // (rID => tID => data) eth in per team, by round id and team id
//****************
// TEAM FEE DATA 
//****************
    mapping (uint256 => LBdatasets.TeamFee) public fees_;          // (team => fees) fee distribution by team
    mapping (uint256 => LBdatasets.PotSplit) public potSplit_;     // (team => fees) pot split distribution by team
//==============================================================================
...
//==============================================================================
        ...
        public
    {
        ...
        // (LB, 0) + (Pot , Referrals, Community)
            // Referrals / Community rewards are mathematically designed to come from the winner's share of the pot.
        fees_[0] = LBdatasets.TeamFee(30,0);   //46% to pot, 20% to aff, 2% to com, 2% to air drop pot
        fees_[1] = LBdatasets.TeamFee(43,0);   //33% to pot, 20% to aff, 2% to com, 2% to air drop pot
        fees_[2] = LBdatasets.TeamFee(56,0);  //20% to pot, 20% to aff, 2% to com, 2% to air drop pot
        fees_[3] = LBdatasets.TeamFee(43,8);   //33% to pot, 20% to aff, 2% to com, 2% to air drop pot
        
        // how to split up the final pot based on which team was picked
        // (LB, 0)
        potSplit_[0] = LBdatasets.PotSplit(15,0);  //48% to winner, 25% to next round, 12% to com
        potSplit_[1] = LBdatasets.PotSplit(20,0);   //48% to winner, 20% to next round, 12% to com
        potSplit_[2] = LBdatasets.PotSplit(25,0);  //48% to winner, 15% to next round, 12% to com
        potSplit_[3] = LBdatasets.PotSplit(30,0);  //48% to winner, 10% to next round, 12% to com
	}
//==============================================================================
//     _ _  _  _|. |`. _  _ _  .
