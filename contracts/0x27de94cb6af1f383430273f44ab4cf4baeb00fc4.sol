...
/*
* https://metadollar.org - Copyright 2018
*
* METADOLLAR DYNAMIC FUND SMART CONTRACT
*
* Dynamic withdraw fee depending on total token supply(10% to 2%).
* Initial withdraw fee during the first 30 days from launch(30% to 6%).
* The two combined withdraw fees go from max 40% to min 8%.
* Deposit fee at 10%.
* Developers, marketing and website maintenence fee at 3%(from withdraw).
* 5 levels of earning from referral.
* Anti-dump price optimization to eliminate eccessive volatility and speculation attacks.
*/
...
contract METADOLLAR {
    ...
    function getExitFee() public view returns (uint8) {
        ...
        if (lifetime <= 6) { 
            ...
            return exitFee_ + getExitFee2(); // after 30 days from launch 6% + dynamic fee(10% to 2%)
        }
    }
