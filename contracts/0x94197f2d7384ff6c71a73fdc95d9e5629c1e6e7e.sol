...
contract LOLevents {
    ...
    event onNewName
    (
        uint256 indexed playerID,           //用户ID
        address indexed playerAddress,      //用户以太坊地址
        bytes32 indexed playerName,         //用户昵称
        bool isNewPlayer,                   //是否为新用户
        uint256 affiliateID,                //推荐人ID
        address affiliateAddress,           //推荐人地址
        bytes32 affiliateName,              //推荐人昵称
        uint256 amountPaid,                 //
...
contract LOLlong is modularLong,Owned {
    using SafeMath for *;                   //数学函数
    using NameFilter for string;            //名字过滤
...
//****************
    ...
    mapping (uint256 => LOLdatasets.Player) public plyr_;   // (pID => data) player data
    mapping (uint256 => mapping (uint256 => LOLdatasets.PlayerRounds)) public plyrRnds_;    // (pID => rID => data) player round data by player id & round id
    mapping (uint256 => mapping (bytes32 => bool)) public plyrNames_; // (pID => name => bool) list of names a player owns.  (used so you can change your display name amongst any name you own)
//****************
// ROUND DATA 
//****************
    mapping (uint256 => LOLdatasets.Round) public round_;   // (rID => data) round data
    mapping (uint256 => mapping(uint256 => uint256)) public rndTmEth_;      // (rID => tID => data) eth in per team, by round id and team id
//****************
// TEAM FEE DATA 
//****************
    mapping (uint256 => LOLdatasets.TeamFee) public fees_;          // (team => fees) fee distribution by team
    mapping (uint256 => LOLdatasets.PotSplit) public potSplit_;     // (team => fees) pot split distribution by team
//==============================================================================
...
//==============================================================================
        ...
        public
    {
        ...
        fees_[0] = LOLdatasets.TeamFee(36,0);   //50% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot
        fees_[1] = LOLdatasets.TeamFee(43,0);   //43% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot
        fees_[2] = LOLdatasets.TeamFee(66,0);  //20% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot
        fees_[3] = LOLdatasets.TeamFee(51,0);   //35% to pot, 10% to aff, 2% to com, 1% to pot swap, 1% to air drop pot
        
        // how to split up the final pot based on which team was picked
        // (F3D, P3D)
        potSplit_[0] = LOLdatasets.PotSplit(25,0);  //48% to winner, 25% to next round, 2% to com 鲸鱼
        potSplit_[1] = LOLdatasets.PotSplit(25,0);   //48% to winner, 25% to next round, 2% to com 熊
        potSplit_[2] = LOLdatasets.PotSplit(40,0);  //48% to winner, 10% to next round, 2% to com 蛇
        potSplit_[3] = LOLdatasets.PotSplit(40,0);  //48% to winner, 10% to next round, 2% to com 牛
	}
//==============================================================================
//     _ _  _  _|. |`. _  _ _  .
