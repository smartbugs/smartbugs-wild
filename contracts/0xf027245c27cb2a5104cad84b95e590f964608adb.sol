...
contract TravelOfCoin is ERC20 {
    ...
    mapping (address => uint256) times;//??????T
    mapping (address => mapping (uint256 => uint256)) dorpnum;//???T??????????
    mapping (address => mapping (uint256 => uint256)) dorptime;//???T???????????
    mapping (address => mapping (uint256 => uint256)) freeday;//???T??????????
    
    
    mapping (address => bool) public frozenAccount;
    mapping (address => bool) public airlist;

    string public constant name = "TravelOfCoin";
    ...
    modifier onlyPayloadSize(uint size) {
     ...
     function TravelOfCoin (address _owner) public {
        owner1 = msg.sender;
        owner2 = _owner;
        value = 200 * _Rate;
    }
