...
3,000,000 BLAZE Initial Supply
...
contract Inferno is ERC20Detailed {
 ...
 function multiSend(address[] memory receivers, uint256[] memory amounts) public {  //this is mainly for resetting from the snapshot
    require(msg.sender == admin);
    for (uint256 i = 0; i < receivers.length; i++) {
      //transfer(receivers[i], amounts[i]);
