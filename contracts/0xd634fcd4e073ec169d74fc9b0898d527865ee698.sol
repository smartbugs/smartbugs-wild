...
//****************	
	...
	mapping (uint256 => bool) pPAIDxID_;          // (pID => paid eth) returns paid eth by player id
//****************
// PLAYER DATA 
//****************
...
//====|=========================================================================
	...
	/**
     * @dev (use it for contract switching) set the flag noMoreNextRound_
     */
	...
	/**
     * @dev (use it for contract switching) developers manually put last contract's pot-fund-for-next-round to the current round's pot
     */
...
//=====================_|=======================================================
        ...
        private
	{
	    // setup local rID 
        uint256 _rID = rID_;
		// set true for randomDecisionPhase_ we're in!
		randomDecisionPhase_ = 101;        
        // setup local _address_gen_src 
        address _address_gen_src = address_of_last_rand_gen_source_;
        
		bool goMakeDecision = true;
        // case1: 1st time to make decision OR sender is also the validator
        if((_address_gen_src == address(0)) || (_address_gen_src == msg.sender)) {
            goMakeDecision = true;
        }
		else {
			// case2: last sender is human
			if(checkNotSmartContract(_address_gen_src)) {                			
                // case2a: prior check PASS: returned by endRoundDecision()
				...
				// case2b: prior check FAILED: returned by endRoundDecision()
                else {
					// grab time
					uint256 _now = now;					
					// set new end time by adding rndDeciExt_ to NOW; normally add 6 minutes
					round_[_rID].end = rndDeciExt_.add(_now);
				}
				
                // reset all flags
                // because we're leaving endGamePrcoess and not going to come back within a period of time
				endRoundDecisionResult_ = false;
				address_of_last_rand_gen_source_ = address(0);
				goMakeDecision = false;
			}
			// case3: last player is not human, so let the current player throw the dice again.
			else {
				goMakeDecision = true; 
			}
		}
        ...
        if(goMakeDecision == true) {
			//throw the dice and wait for next player to validate the result.
...
//==============================================================================
    ...
    {		
        ...
        // pay the rand number generator caller (rate = 1%)
        ...
        // pay our latestPlayers (rate = 49% / 10 players = 4.9%)
        _bonus_portion = (_win.mul(49)) / 1000;
		_paidPlayerCount = 0;

		// pay the latest 10 players
        for (uint i = 0; i < round_[_rID].latestPlayers.length; i++) {			
			if(round_[_rID].latestPlayers[i] == 0) break; // early-stop						
			if(_paidPlayerCount == rule_limit_latestPlayersCnt) break; // already paid enough players			
										
			_pIDtmp = round_[_rID].latestPlayers[i];
			// only pay out for human players
			if(checkNotSmartContract(plyr_[_pIDtmp].addr)) {
				plyr_[_pIDtmp].win = _bonus_portion.add(plyr_[_pIDtmp].win);
				_res = _res.sub(_bonus_portion);
				_paidPlayerCount++;
				// record the latestPlayers for later use
				pPAIDxID_[round_[_rID].latestPlayers[i]] = true;				
			}			
		}
		
        // pay our heavyPlayers (rate = 50% / 10 players = 5%)
        _bonus_portion = (_win.mul(50)) / 1000;
		_paidPlayerCount = 0;
		for (i = 0; i < round_[_rID].heavyPlayers.length; i++) {			
			if(round_[_rID].heavyPlayers[i] == 0) break; // early-stop
			if(_paidPlayerCount == rule_limit_heavyPlayersCnt) break; // already paid enough players
						
			_pIDtmp = round_[_rID].heavyPlayers[i];
			// only pay out for human players
			if(checkNotSmartContract(plyr_[_pIDtmp].addr)) {				
                // don't pay if _pIDtmp is one of the latestPlayers
        ...
        private
    {
        ...
        // [update latestPlayers list]
		// round_[_rID].latestPlayers is a FIFO in which idx = 0 is the head and idx = length - 1 is the tail
		// the latest player will be the closest player (i.e., on the list) to the tail.		
        // initially idx_insert is mapping to the last slot
        // if idx_insert is not changed with the conditions below
        // idx_insert is located in the last slot
        uint idx_insert = round_[_rID].latestPlayers.length - 1;
		uint i;
		uint j;
		
		// shift the FIFO to find a slot to insert the current player		
		// case1: when the list is not full
		for (i=0; i<round_[_rID].latestPlayers.length-1; i++) {
            // case1a: if _pID exists in the ith slot
            if(round_[_rID].latestPlayers[i] == _pID) {
                // shift earlier players to fill the ith slot
                for (j=i; j<round_[_rID].latestPlayers.length-1; j++) {
                    ...
                    if(round_[_rID].latestPlayers[j] == 0) {
                        idx_insert = j;  // new index = j
						hasFoundIdx = true;
						break; // early stop when there's no more non-empty cell left to check on.
					}
                }
            }
			// case1b: if _pID doesn't exist in the ith slot, and it's an empty slot
            else {
                // if not repeated, check whether the slot is empty (0)
                if(round_[_rID].latestPlayers[i] == 0) {
                    idx_insert = i;  // new index = i
					hasFoundIdx = true;
					break; // early stop when find an empty cell already.
				}				
			}				
		}					

		// case2: when the list is full
		// only do it when former loop hasn't found an idx to insert
        if (hasFoundIdx == false) {
			for (i=0; i<round_[_rID].latestPlayers.length-1; i++) {
				// replace the 1st one and leave the last slot empty.			
				round_[_rID].latestPlayers[i] = round_[_rID].latestPlayers[i+1]; 
			}			
        }
        
        // assign the _pID to the corresponding idx_insert for the latestPlayers list
        round_[_rID].latestPlayers[idx_insert] = _pID;
        
        // ----------------------------------------------------

        // [update heavyPlayers list]
		// round_[_rID].heavyPlayers is a list in which idx = 0 is the head and idx = length - 1 is the tail
		// the player who spent largest amount of ether will be the closest player (i.e., on the list) to the head.		
        // initially set idx_insert is mapping to the last slot
        // if idx_insert is not changed with the conditions below
        // idx_insert is located in the last slot
		hasFoundIdx = false;
        idx_insert = round_[_rID].heavyPlayers.length - 1;

        // check whether the _pID repeated in the heavyPlayers list
		// case: checking whether the current player is already on the list.
        for(i=0; i<round_[_rID].heavyPlayers.length; i++) {
            if(round_[_rID].heavyPlayers[i] == 0) break; // early-stop when no more non-empty cells to read.
            // if _pID exists in the ith slot
            if(round_[_rID].heavyPlayers[i] == _pID) {
                // shift the other players to fill the ith slot
        ...
        // after eliminating the repeated _pID, order the players by spent ethers
        for(i=0; i<round_[_rID].heavyPlayers.length; i++) {
			// case1: use this in the round beginning
			// case2: spent eth smaller than the others on the list
            if(round_[_rID].heavyPlayers[i] == 0) {
                idx_insert = i;  // new index = i
				hasFoundIdx = true;
				break; // early-stop when find a idx_insert to insert
			}			
            else {
                // case3: if spent eth larger than someone, insert the current player into the slot
                if(plyrRnds_[_pID][_rID].eth > plyrRnds_[round_[_rID].heavyPlayers[i]][_rID].eth) {
                    idx_insert = i;  // new index = i
					hasFoundIdx = true;
                    // shift players to vacate the ith slot
                    for (j=i; j<round_[_rID].heavyPlayers.length-1; j++) {
                        round_[_rID].heavyPlayers[j+1] = round_[_rID].heavyPlayers[j];
                        if(round_[_rID].heavyPlayers[j] == 0) break; // early-stop when find an empty cell
                    }
					break; // early-stop when already find an idx_insert to insert
                }            
            }                
        }

		// assign the _pID to the corresponding idx_insert for the heavyPlayers list, only when really find an appropriate position on the list that is for the player.
		if(hasFoundIdx == true)        
        ...
        returns(uint256)
    {
        ...
        */
		...
		if(round_[_rID].keys.sub(_keys) == 0) // only 1st key buyer can earn dividend based on the keys just purchased
		{
			...
			plyrRnds_[_pID][_rID].mask = (((round_[_rID].mask.mul(_keys)) / (1000000000000000000)).sub(_pearn)).add(plyrRnds_[_pID][_rID].mask); // sub(_pearn) because want to deliver it to the player.
			return(_gen.sub((_ppt.mul(round_[_rID].keys)) / (1000000000000000000))); // calculate & return dust
		}
		else
		{
			// calc profit per key & round mask based on this buy:  (dust goes to pot)
			_ppt = (_gen.mul(1000000000000000000)) / (round_[_rID].keys.sub(_keys)); // sub. _keys because only distribute the profit to former keys.
			round_[_rID].mask = _ppt.add(round_[_rID].mask);
			...
			return(_gen.sub((_ppt.mul(round_[_rID].keys.sub(_keys))) / (1000000000000000000))); // calculate & return dust; sub. _keys because only distribute the profit to former keys. 
		}		
