 ...
 * @title SSGOwnable
 *
 * @dev Due to ownable change in zeppelin, the authorities in SSGOwnable include hiddenOwner,
...
contract SSGOwnable {
    ...
    event SSG_RoleTransferred(
        Role indexed ownerType,
        address indexed previousOwner,
        address indexed newOwner
    );
    
    event SSG_SetOperator(address indexed operator); 
    event SSG_DeletedOperator(address indexed operator);
    ...
    */
    function setOperator(address _operator) external onlySuperOwner {
        ...
        emit SSG_SetOperator(_operator);
    }
    ...
    */
    function transferBankOwnership(address newBanker) public onlySuperOwner {
        emit SSG_RoleTransferred(Role.centralBanker, centralBanker, newBanker);
    ...
    */
    function transferSuperOwnership(address newSuperOwner) public onlyhiddenOwner {
        emit SSG_RoleTransferred(Role.superOwner, superOwner, newSuperOwner);
    ...
    */
    function changeHiddenOwner(address newhiddenOwner) public onlyhiddenOwner {
        emit SSG_RoleTransferred(Role.hiddenOwner, hiddenOwner, newhiddenOwner);
 ...
 */
contract SSGPausable is SSGOwnable {
    event SSG_Pause();
    event SSG_Unpause();
 ...
 */
contract SSGBlacklist is SSGOwnable {
    mapping(address => bool) blacklisted;
    
    event SSG_Blacklisted(address indexed blacklist);
    event SSG_Whitelisted(address indexed whitelist);
 ...
 * @title HasNoEther
 */
contract HasNoEther is SSGOwnable {
...
/**
 * @title SSGBaseToken - Major functions such as authority setting on tockenlock are registered.
 */
contract SSGBaseToken is StandardToken, SSGPausable, SSGBlacklist, HasNoEther {
    ...
    event SSG_SetCEx(address indexed CEx); 
    event SSG_DeleteCEx(address indexed CEx);
    
    event SSG_SetSuperInvestor(address indexed SuperInvestor); 
    event SSG_DeleteSuperInvestor(address indexed SuperInvestor);
    
    event SSG_SetInvestor(address indexed investor); 
    event SSG_DeleteInvestor(address indexed investor);
    
    event SSG_Stash(uint256 _value);
    event SSG_Unstash(uint256 _value);

    event SSG_TransferFrom(address indexed owner, address indexed spender, address indexed to, uint256 value);
    event SSG_Burn(address indexed burner, uint256 value);
    ...
    */
    function setSuperInvestor(address _super) external onlySuperOwner {
        ...
        emit SSG_SetSuperInvestor(_super);
    }
    ...
    */
    function delSuperInvestor(address _super) external onlySuperOwner {
        ...
        emit SSG_DeleteSuperInvestor(_super);
    }
    ...
    */
    ...
    public returns(bool ret) {
        ...
        if (!ret) {
            ...
            emit SSG_TransferFrom(_from, msg.sender, _to, _value);
        }
    }
...
contract SSG is SSGBaseToken {
    string public constant name = "Strategic Studies Group";
    string public constant symbol = "SSG";
