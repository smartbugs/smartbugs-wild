...
contract Ethex {
    ...
    function sellOrderBalances(bytes32 orderHash) external returns (uint256); //Returns number of tokens - e.g. available maker tokens
    function buyOrderBalances(bytes32 orderHash) external returns (uint256); //Returns number of eth - e.g. available maker's eth
