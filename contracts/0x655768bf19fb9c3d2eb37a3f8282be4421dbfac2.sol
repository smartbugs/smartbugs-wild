...
contract GGToken is BurnableToken, UpgradeableToken {
  ...
  function GGToken(address _owner, string _name, string _symbol, uint _totalSupply, uint _decimals) public UpgradeableToken(_owner) {
    name = _name;
    symbol = _symbol;
    decimals = _decimals;
...
contract GGTVestingForPrivateInvestor {
    address public investorAddress;
    uint public deadline;

    GGToken public tokenReward;
    ...
    modifier onlyInvestor() {
        if(msg.sender != investorAddress) revert();
        _;
    }

    modifier onlyWhenItsAllowed() {
        if(now < deadline) revert();
        _;
    }

    function GGTVestingForPrivateInvestor(
        address investor,
        GGToken addressOfTokenUsedAsReward,
        uint end
    ) {
        investorAddress = investor;
        tokenReward = GGToken(addressOfTokenUsedAsReward);
        deadline = end;
    }

    function TransferMyGGTokens() onlyInvestor onlyWhenItsAllowed {
      tokenReward.transfer(msg.sender, tokenReward.balanceOf(this));
    }

    function GetInvestorAddress() returns (address) {
