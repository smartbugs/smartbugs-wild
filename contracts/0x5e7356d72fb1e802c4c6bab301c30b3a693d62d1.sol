            ...
            if (respectIntersections) {
                ...
                for (uint j = 0; j < i; j++) {
                    ...
                    if (orders[j].makerAddress == orders[i].makerAddress) {
                        ...
                        uint256 orderMakerAssetFilledAmount = ordersInfo[j].orderTakerAssetFilledAmount
                            .mul(orders[j].makerAssetAmount)
                            .div(orders[j].takerAssetAmount);

                        uint256 orderMakerAssetRemainigAmount = orders[j].makerAssetAmount
                            .sub(orderMakerAssetFilledAmount);

                        if (possibleMakerAmount > orderMakerAssetRemainigAmount) {
                            possibleMakerAmount = possibleMakerAmount.sub(orderMakerAssetRemainigAmount);
                        } else {
                            possibleMakerAmount = 0;
                            break;
                        }
                    }
                }
