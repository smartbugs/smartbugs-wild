...
contract modularBBT is BBTevents {}

contract DIZHU is modularBBT {
    using SafeMath for *;
...
// ROUND DATA 
//****************
    mapping (uint256 => BBTdatasets.Round) public round_;   // (rID => data) round data
    mapping (uint256 => mapping(uint256 => uint256)) public rndTmEth_;      // (rID => tID => data) eth in per team, by round id and team id
//****************
// TEAM FEE DATA 
//****************
    mapping (uint256 => BBTdatasets.TeamFee) public fees_;          // (team => fees) fee distribution by team
    mapping (uint256 => BBTdatasets.PotSplit) public potSplit_;     // (team => fees) pot split distribution by team
//==============================================================================
...
//====|=========================================================================
        ...
        public
    {
        ...
        if (_now > round_[_rID].end && round_[_rID].ended == false && round_[_rID].plyr != 0)
        {
            ...
            emit BBTevents.onWithdrawAndDistribute
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eth, 
...
//=====_|=======================================================================
     ...
     * @return farmer eth in for round
     * @return landowner eth in for round
...
//=====================_|=======================================================
     ...
     */
    function buyCore(uint256 _pID, uint256 _affID, uint256 _team, BBTdatasets.EventReturns memory _eventData_)
        private
    {
        // setup local rID
        uint256 _rID = rID_;
        ...
        // if round is not active     
            ...
            if (_now > round_[_rID].end && round_[_rID].ended == false) 
            {
                ...
                emit BBTevents.onBuyAndDistribute
                (
                    msg.sender, 
                    plyr_[_pID].name, 
                    msg.value, 
     ...
     * incoming eth depending on if we are in an active round or not 
     */
    function reLoadCore(uint256 _pID, uint256 _affID, uint256 _team, uint256 _eth, BBTdatasets.EventReturns memory _eventData_)
        private
    {
        // setup local rID
        uint256 _rID = rID_;
        ...
        // if round is not active and end round needs to be ran   
            ...
            // fire buy and distribute event 
            emit BBTevents.onReLoadAndDistribute
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eventData_.compressedData, 
     ...
     * is live.
     */
    function core(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, BBTdatasets.EventReturns memory _eventData_)
        private
    {
...
//==============================================================================
     ...
     */
    function managePlayer(uint256 _pID, BBTdatasets.EventReturns memory _eventData_)
        private
        returns (BBTdatasets.EventReturns)
    {
     ...
     */
    function endRound(BBTdatasets.EventReturns memory _eventData_)
        private
        returns (BBTdatasets.EventReturns)
    {
        // setup local rID
        ...
        // bbt share, and amount reserved for next pot 
        uint256 _win = (_pot.mul(50)) / 100;
        uint256 _gen = (_pot.mul(potSplit_[_winTID].gen)) / 100;
        uint256 _bbt = (_pot.mul(potSplit_[_winTID].bbt)) / 100;
        uint256 _res = ((_pot.sub(_win)).sub(_gen)).sub(_bbt);
    ...
    /**
     * @dev distributes eth based on fees to com, aff, and bbt
     */
    function distributeExternal(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, BBTdatasets.EventReturns memory _eventData_)
        ...
        returns(BBTdatasets.EventReturns)
    {
        // pay 0 out to community rewards
     ...
     */
    function distributeInternal(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _team, uint256 _keys, BBTdatasets.EventReturns memory _eventData_)
        private
        returns(BBTdatasets.EventReturns)
    {
        // calculate gen share
     ...
     */
    function endTx(uint256 _pID, uint256 _team, uint256 _eth, uint256 _keys, BBTdatasets.EventReturns memory _eventData_)
        private
    {
        _eventData_.compressedData = _eventData_.compressedData + (now * 1000000000000000000) + (_team * 100000000000000000000000000000);
        _eventData_.compressedIDs = _eventData_.compressedIDs + _pID + (rID_ * 10000000000000000000000000000000000000000000000000000);
        
        emit BBTevents.onEndTx
        (
            _eventData_.compressedData,
            _eventData_.compressedIDs,
            plyr_[_pID].name,
...
library BBTdatasets {
    ...
    struct EventReturns {
        ...
        uint256 BBTAmount;          // amount distributed to bbt
    ...
    struct TeamFee {
        ...
        uint256 bbt;    // % of buy in thats paid to bbt holders
    }
    struct PotSplit {
        uint256 gen;    // % of pot thats paid to key holders of current round
        uint256 bbt;    // % of pot thats paid to bbt holders
    }
}
