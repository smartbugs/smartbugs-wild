...
contract ICO is StagedCrowdsale, CommonSale {
  ...
  function calculateTokens(uint _invested) internal returns(uint) {
    ...
    uint tokens = _invested.mul(price).div(0.001 ether);
...
contract PreICO is NextSaleAgentFeature, WhiteListFeature {
  ...
  function calculateTokens(uint _invested) internal returns(uint) {
    return _invested.mul(price).div(0.001 ether);
  }
...
// File: contracts/ripplegold.sol

contract ripplegold is MintableToken {

  string public constant name = "RIPPLEGOLD";

  string public constant symbol = "XRPG";
...
contract Configurator is Ownable {
  ...
  function deploy() public onlyOwner {
    ...
    preICO.setStart(1520640000); // 18 apr 2018 00:00:00 GMT
    preICO.setPeriod(22);
    ...
    ico.setStart(1522627200); // 30 mai 2018 00:00:00 GMT
