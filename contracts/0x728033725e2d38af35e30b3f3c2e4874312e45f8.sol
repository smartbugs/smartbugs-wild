...
/**
  ...
  5. Баланс этого контракта должен обычно быть в районе 0, потому что все поступления
     ...
     Таким образом, последние платят первым, и инвесторы, достигшие выплат 200% от депозита,
...
contract Multiplier {
    ...
    uint constant public PROMO_PERCENT = 10; //9 for advertizing, 1 for techsupport
    ...
    struct Deposit {
        ...
        uint128 expect;    //How much we should pay out (initially it is 200% of deposit)
    }
    ...
    function () public payable {
        if(msg.value > 0){
            ...
            //Add the investor into the queue. Mark that he expects to receive 200% of deposit back
