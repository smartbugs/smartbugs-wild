...
/** @title main contract for met token. we should deploy this contract. needs about 5,500,000 gas */
...
contract MetToken is Z_StandardToken, Z_Ownable {
    ...
    // total token supply: 1000 billion met
    ...
    // array of token sale price for each stage (wei per met)
    ...
    struct history_token_transfer_obj {
      address _from;
      address _to;
      uint256 _token_value; // in met token
      uint256 _when; 
    }
    ...
    struct history_token_burning_obj {
      address _from;
      uint256 _token_value_burned; // in met token
      uint256 _when; 
    }
     ...
     * return  _how_many_wei_per_met   new token sale price (wei per met)
     */
     ...
     * return  _sale_price   get current token sale price (wei per met)
    ...
    /**
     *  @dev   buy met tokens by sending some ethers  to this contract address
