...
contract QuipToken is MintableToken {
  ...
  function devTeamMint(address[3] _team, uint256 _amount) onlyOwner canMint public {
    require(singleDevMint == false);
    require(totalSupply.add(_amount * 3) <= MAX_TOKEN);
    super.mint(_team[0], _amount);
    super.mint(_team[1], _amount);
    super.mint(_team[2], _amount);
    singleDevMint = true;
  }
