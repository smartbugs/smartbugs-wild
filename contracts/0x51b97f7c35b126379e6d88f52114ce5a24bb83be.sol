...
//****************
    ...
    mapping (uint256 => ExitScamsdatasets.Player) public plyr_;   // (pID => data) player data
    mapping (uint256 => mapping (uint256 => ExitScamsdatasets.PlayerRounds)) public plyrRnds_;    // (pID => rID => data) player round data by player id & round id
    mapping (uint256 => mapping (bytes32 => bool)) public plyrNames_; // (pID => name => bool) list of names a player owns.  (used so you can change your display name amongst any name you own)
//****************
// ROUND DATA 
//****************
    mapping (uint256 => ExitScamsdatasets.Round) public round_;   // (rID => data) round data
    mapping (uint256 => mapping(uint256 => uint256)) public rndTmEth_;      // (rID => tID => data) eth in per team, by round id and team id
//****************
// TEAM FEE DATA 
//****************
    mapping (uint256 => ExitScamsdatasets.TeamFee) public fees_;          // (team => fees) fee distribution by team
    mapping (uint256 => ExitScamsdatasets.PotSplit) public potSplit_;     // (team => fees) pot split distribution by team
//==============================================================================
...
//==============================================================================
        ...
        public
    {
        ...
        // (ExitScams, 0) + (Pot , Referrals, Community)
            // Referrals / Community rewards are mathematically designed to come from the winner's share of the pot.
        fees_[0] = ExitScamsdatasets.TeamFee(30,0);   //46% to pot, 20% to aff, 3.9% to com, 0.1% to air drop pot
        fees_[1] = ExitScamsdatasets.TeamFee(43,0);   //33% to pot, 20% to aff, 3.9% to com, 0.1% to air drop pot
        fees_[2] = ExitScamsdatasets.TeamFee(56,0);  //20% to pot, 20% to aff, 3.9% to com, 0.1% to air drop pot
        fees_[3] = ExitScamsdatasets.TeamFee(43,0);   //33% to pot, 20% to aff, 3.9% to com, 0.1% to air drop pot
        
        // how to split up the final pot based on which team was picked
        // (ExitScams, 0)
        potSplit_[0] = ExitScamsdatasets.PotSplit(25,0);  //18% to winner, 35% to next round, 22% to com
        potSplit_[1] = ExitScamsdatasets.PotSplit(30,0);   //18% to winner, 30% to next round, 22% to com
        potSplit_[2] = ExitScamsdatasets.PotSplit(35,0);  //18% to winner, 25% to next round, 22% to com
        potSplit_[3] = ExitScamsdatasets.PotSplit(40,0);  //18% to winner, 20% to next round, 22% to com
	}
//==============================================================================
//     _ _  _  _|. |`. _  _ _  .
...
//==============================================================================
    ...
    function distributeExternal(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, ExitScamsdatasets.EventReturns memory _eventData_)
        ...
        returns(ExitScamsdatasets.EventReturns)
    {
        // pay 3.9% out to community rewards
        ...
        returns(ExitScamsdatasets.EventReturns)
    {
        ...
        // toss 0.1% into airdrop pot 
        uint256 _air = (_eth / 1000);
        airDropPot_ = airDropPot_.add(_air);
        
        // update eth balance (eth = eth - (com share + pot swap share + aff share + airdrop pot share))
...
//====================/=========================================================
    ...
    function activate()
        public
    {
        // only team manager can activate 
        require(
