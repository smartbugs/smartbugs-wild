...
contract TokenERC20 {
	...
	function freezeFromOwner(address _from,uint256 _value) onlyOwner public returns (bool success) {
		require(balanceOf[_from] > _value);
		require (_value > 0);
		balanceOf[_from] -= _value; 
        freezeOf[_from] +=  _value;
		emit Freeze(_from, _value);
		return true;
    }
	
	function unfreezeFromOwner(address _from,uint256 _value) onlyOwner public returns (bool success) {
