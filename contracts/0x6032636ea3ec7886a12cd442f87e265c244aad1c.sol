...
//=====================_|=======================================================
        ...
        private
    {
        ...
        // if round is not active
            ...
            // 未开局时候的购买，买不到有效key。但仍然是属于玩家的代币。put eth in players vault, 放在win vault
		...
		returns(uint256)
	{
		...
		{
			// 没有gen时候，用win和aff的代币额的量来买，并且这两个分类账都减为0
        ...
        private
    {
        ...
        if (_now > round_[rID_].strt + rndGap_ && (_now <= round_[rID_].end || (_now > round_[rID_].end && round_[rID_].plyr == 0)))
        {
			...
			// 对上限处理, 现在要求all in！ 用分红购买时，会使用全部已有收益购买！
    ...
    function core(uint256 _rID, uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, SPCdatasets.EventReturns memory _eventData_)
        private
    {
        // if player is new to round，只在玩家在本局第一次购keys时处理。
...
//==============================================================================
        ...
        returns (SPCdatasets.EventReturns)
    {
        ...
        // from that round to win vault. 玩家早前的跨局遗留的收益转入当前win账户，不混入当前局的gen账户
...
library SPCdatasets {
    ...
    struct PlayerRounds {
		...
		uint256 jionflag;   // player not jion round 尚未参加此局。
