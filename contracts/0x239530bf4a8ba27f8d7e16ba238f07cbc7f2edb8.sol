...
/*
*===============================================================================================
* POWH5D P5D - https://p5d.io
* ==============================================================================================
* THE ULTIMATE EXPANSION FOR THE POWH3D IMPROVED WITH:
* 1) DYNAMIC BUY FEES - 20% - 10% DEPENDING ON TOTAL TOKEN SUPPLY
* 2) DYNAMIC BUY FEES - 10% - 2% DEPENDING ON TOTAL TOKEN SUPPLY
* 3) TOKEN PRICE INCREMENTAL AS THE ORIGINAL P3D
* ==============================================================================================
...
*      -> transferFrom(): use your approval allowance to transfer P5D on anothers behalf
...
*  [x] Masternodes are also used in P5D as well as when it buys P3D:
*      -> If the referrer has more than 10,000 P5D tokens, they will get 1/3 of the 10% divs
...
*      holdings, this ETH is then distributed to all P5D token holders proportionally
...
*      -> All ETH from registrations will be distributed to all P5D holders proportionally
*
* =================================================================================================
*/
...
contract P5D {
    ...
    // -> allow a contract to accept P5D tokens
    // they CANNOT:
    // -> take funds
    // -> disable withdrawals
    // -> kill the contract
    ...
    uint256 constant internal buyDividendFee_ = 10; // 10% maximum dividend tax on each buy
    uint256 constant internal buyDividendFee2_ = 20; // 20% minimum dividend tax on each buy
    uint256 constant internal sellDividendFee_ = 2; // 5% minimum dividend tax on each sell
    uint256 constant internal sellDividendFee2_ = 10; // 10% maximum dividend tax on each sell
    uint256 internal tokenPriceInitial_; // set in the constructor
    uint256 constant internal tokenPriceIncremental_ = 1e8; // same incremental of P3D
    uint256 constant internal magnitude = 2**64;
    uint256 public stakingRequirement = 1e22; // 10,000 P5D
    ...
    */
    constructor(uint256 _activationTime, address _P3D_address) public {
        ...
        // calculateTokensReceived() for this contract will return how many tokens can be bought starting at 1e9 P3D per P5D
        // as the price increases by the incremental each time we can just multiply it out and scale it back to e18
        //
        // this is used as the initial P3D-P5D price as it makes it fairer on other investors that aren't ambassadors
        uint256 _P3D_received;
    ...
    function getExitFee() public view returns (uint256) {
        ...
        if (tsupply <= 25e22) { 
            ...
            return (uint8) (sellDividendFee2_  - 2); // 8%
        } else if (tsupply > 75e22 && tsupply <= 1e24) {
            return (uint8) (sellDividendFee2_  - 3); // 7%
        } else if (tsupply > 1e24 && tsupply <= 125e22) {
            return (uint8) (sellDividendFee2_  - 4); // 6%
        } else if (tsupply > 125e22 && tsupply <= 15e24) {
            return (uint8) (sellDividendFee2_  - 5); // 5%
        } else if (tsupply > 15e23 && tsupply <= 2e24) {
            return (uint8) ( sellDividendFee2_ - 6); // 4%
        } else if (tsupply > 2e24 && tsupply <= 3e24) {
            return (uint8) (sellDividendFee2_  - 7); // 3%
        } else {
    ...
    function getEntryFee() public view returns (uint256) {
        ...
        if (tsupply <= 100000e18) { 
            ...
            return (uint8) (buyDividendFee2_  - 2); // 18%
        } else if (tsupply > 3e23 && tsupply <= 4e23) {
            return (uint8) (buyDividendFee2_  - 3); // 17%
        } else if (tsupply > 4e23 && tsupply <= 5e23) {
            return (uint8) (buyDividendFee2_  - 4); // 16%
        } else if (tsupply > 5e23 && tsupply <= 1e24) {
            return (uint8) (buyDividendFee2_  - 5); // 15%
        } else if (tsupply > 1e24 && tsupply <= 2e24) {
            return (uint8) (buyDividendFee2_  - 6); // 14%
        } else if (tsupply > 2e24 && tsupply <= 3e24) {
            return (uint8) (buyDividendFee2_  - 7); // 13%
        } else if (tsupply > 3e24 && tsupply <= 4e24) {
            return (uint8) (buyDividendFee2_  - 8); // 12%
        } else if (tsupply > 4e24 && tsupply <= 5e24) {
            return (uint8) (buyDividendFee2_  - 9); // 11%
        } else {
    ...
    /**
     * Allows a customer to pay for a global name on the P5D network
     * There's a 0.01 ETH registration fee per name
     * All ETH is distributed to P5D token holders via updateSubdivsFor()
     */
        ...
        public
    {
        ...
        // to all P5D tokens holders via the updateSubdivsFor() method
    }
    ...
    /**
     * Add a sub-contract, which can accept P5D tokens
     */
    ...
    */
    ...
    // This function should always be called before a customers P5D balance changes.
    ...
    // all of the P5D token holders proportionally.
