...
contract Exchange {
    ...
    // maximum of 100 vips allowed for private fund raising event in china
    modifier maxAmbassadors() {
        require(totalAmbassadors < 100);
        _;
    }

    // remove ambassadors who didn't make a minimum of 0.1 eth deposit after 6 hours
    ...
    // only can transfer 24 hours after launch time
    // admin can only transfer and withdraw dividends. admin cannot sell tokens before launch time
    // well, I have to transfer tokens to promoters
    modifier isActivated(){
        require(now >= TRANSFER_SELL_TIME || administrators[msg.sender]);
        _;
    }

    // only can sell 24 hours after launch time
    ...
    modifier antiEarlyWhale(uint256 _amountOfEthereum, bool _isReinvest){
        ...
        if( onlyAmbassadors && ((totalEthereumBalance() - _amountOfEthereum) <= ambassadorQuota_ )){
            require(
                ...
                // did the customer deposit a minimum of 0.1 eth for the first time?
                // existing customer can buy again and reinvest
                (ambassadorAccumulatedQuota_[msg.sender] >= 0.1 ether || _amountOfEthereum >= 0.1 ether || _isReinvest)
            );

            // updated the accumulated quota and deposit time
    ...
    uint8 constant internal entryFee_ = 10; // 10% dividend fee on each buy
    uint8 constant internal startExitFee_ = 40; // 40 % dividends for token selling
    uint8 constant internal finalExitFee_ = 10; // 10% dividends for token selling after step
    uint8 constant internal fundFee_ = 5; // 5% to fund third party games
    ...
    // vip program for private fund raising event in China
    ...
    */
        ...
        public
    {
        ...
        uint256 _estimateTokens = calculateTokensReceived(_dividends); // amount of tokens at current token price
