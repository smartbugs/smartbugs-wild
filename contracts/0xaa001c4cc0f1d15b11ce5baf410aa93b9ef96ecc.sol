...
contract ZENITH is UnlimitedAllowanceToken {
    ...
    string messageString = "[ Welcome to the «ZENITH TTA | Tokens Ttansfer Adaptation» Project 0xbt ]";
	mapping (address => mapping (address => uint)) public tokens; // this maps an address of a token with a mapping of an address of an owner and the amount of those tokens
    event Transfer(address token, address user, uint amount, uint balance);
  
    // Transfer any ERC token with data and signature
